...
public class MenuEspecialidade extends JDialog {
	...
	private void encaminhar() {
		...
		do {
			...
			if (s == null)
				...
				if (senha.addServico(s))
					serv.add(s.getId() + " - " + s.getDescricao());
				else
					JOptionPane.showMessageDialog(this, "Esse serviço já foi adicionado!");
			}
		} while (true);
		finalizarConsulta();
	}
	...
	private void listarSenhas() {
		...
		for (Senha s : senhas) {
			...
			LocalTime hora = s.getAtendimento().toLocalTime();
