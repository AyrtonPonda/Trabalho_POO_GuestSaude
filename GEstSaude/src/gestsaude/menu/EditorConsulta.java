...
public class EditorConsulta extends JDialog {
	...
	private EditorConsulta(Point posicao, GEstSaude g, Consulta consulta, String titulo) {
		...
		List<String> especialidadeIds = new ArrayList<>();
		for (Especialidade e : gest.getEspecialidades()) {
			especialidadeIds.add(e.getId());
		}
		setupAspeto(especialidadeIds);
	...
	protected void testaIdUtente() {
		...
		utente = gest.getUtente(Integer.parseInt(snsUtenteTF.getText()));
		if (utente == null) {
	...
	protected void testaIdServico() {
		...
		Especialidade esp = gest.getEspecialidade(idEscolhido);
		if (esp == null) {
	...
	private void testaTudoOk() {
		if (utente == null)
			...
			Consulta c = new Consulta(utente, especialidade, quando);
	...
	protected void okPremido() {
		...
		// Criar a consulta com os valores escolhidos
		// Se for edição, cria uma nova Consulta; se for criação, usa o utente e especialidade
		if (consulta == null) {
			// Criação de nova consulta
			consultaRes = new Consulta(utente, especialidade, quando);
		} else {
			// Edição de consulta existente - cria uma nova com os novos dados
			consultaRes = new Consulta(utente, especialidade, quando);
		}
		setVisible(false);
	}
